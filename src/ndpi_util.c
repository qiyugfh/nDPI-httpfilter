...
struct ndpi_workflow* ndpi_workflow_init(const struct ndpi_workflow_prefs * prefs,
  ...
  if(workflow->ndpi_struct == NULL) {
    log_error("global structure initialization failed\n");
    exit(-1);
  }
  ...
  *proto = iph->protocol;
  ...
  if(ret == NULL) {
    if(workflow->stats.ndpi_flow_count == workflow->prefs.max_ndpi_flows) {
      log_error("maximum flow count (%u) has been exceeded", workflow->prefs.max_ndpi_flows);
      exit(-1);
    } else {
      struct ndpi_flow_info *newflow = (struct ndpi_flow_info*)malloc(sizeof(struct ndpi_flow_info));

      if(newflow == NULL) {
	    log_error("[NDPI] %s(1): not enough memory", __FUNCTION__);
	    return(NULL);
      } else
      ...
      if((newflow->ndpi_flow = ndpi_flow_malloc(SIZEOF_FLOW_STRUCT)) == NULL) {
    	log_error("[NDPI] %s(2): not enough memory", __FUNCTION__);
    	free(newflow);
    	return(NULL);
      } else
      ...
      if((newflow->src_id = ndpi_malloc(SIZEOF_ID_STRUCT)) == NULL) {
    	log_error("[NDPI] %s(3): not enough memory", __FUNCTION__);
    	free(newflow);
    	return(NULL);
      } else
	memset(newflow->src_id, 0, SIZEOF_ID_STRUCT);
      ...
      if((newflow->dst_id = ndpi_malloc(SIZEOF_ID_STRUCT)) == NULL) {
        log_error("[NDPI] %s(4): not enough memory", __FUNCTION__);
        free(newflow);
        return(NULL);
      } else
...
iph_check:
  ...
  if(type == ETH_P_IP && header->caplen >= ip_offset) {
	...
	if(!workflow->prefs.quiet_mode)
	  log_warn("packet capture size is smaller than packet size, DETECTION MIGHT NOT WORK CORRECTLY");
	  cap_warning_used = 1;
      }
    }
  }
  ...
  if(iph->version == IPVERSION) {
	...
	if(!workflow->prefs.quiet_mode)
	  log_warn("IPv4 fragments are not handled by this demo (nDPI supports them)");
  ...
  v4_warning:
    if(ipv4_warning_used == 0) {
      if(!workflow->prefs.quiet_mode)
        log_warn("only IPv4/IPv6 packets are supported in this demo (nDPI supports both IPv4 and IPv6), all other packets will be discarded");
      ipv4_warning_used = 1;
    }
    workflow->stats.total_discarded_bytes +=  header->len;
    return(nproto);
